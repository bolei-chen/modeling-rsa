...
class Qvalue_Machine:
    ...
    def __init__(self, corpus):
        ...
        p_means = np.reshape(np.array(self.corpus[:, c2i['p_mean']]), (len(self.x_coords), len(self.y_coords))) 
        u_means = np.reshape(np.array(self.corpus[:, c2i['u_mean']]), (len(self.x_coords), len(self.y_coords))) 
        v_means = np.reshape(np.array(self.corpus[:, c2i['v_mean']]), (len(self.x_coords), len(self.y_coords))) 
        gamma1s = np.reshape(np.array(self.corpus[:, c2i['u_mean']] / np.sqrt(self.corpus[:, c2i['u_mean']] ** 2 + self.corpus[:, c2i['v_mean']] ** 2)), (len(self.x_coords), len(self.y_coords))) 
        gamma2s = np.reshape(np.array(self.corpus[:, c2i['v_mean']] / np.sqrt(self.corpus[:, c2i['v_mean']] ** 2 + self.corpus[:, c2i['v_mean']] ** 2)), (len(self.x_coords), len(self.y_coords))) 
        u_mean_squares = np.reshape(np.array([u_mean ** 2 for u_mean in np.array(self.corpus[:, c2i['u_mean']])]), (len(self.x_coords), len(self.y_coords)))
        v_mean_squares = np.reshape(np.array([v_mean ** 2 for v_mean in np.array(self.corpus[:, c2i['v_mean']])]), (len(self.x_coords), len(self.y_coords)))
         
        self.dp_dxs, self.dp_dys = np.gradient(p_means, self.x_coords, self.y_coords) 
        self.du_dxs, self.du_dys = np.gradient(u_means, self.x_coords, self.y_coords) 
        self.dv_dxs, self.dv_dys = np.gradient(v_means, self.x_coords, self.y_coords) 
        self.dus_dxs, self.dus_dys = np.gradient(u_mean_squares, self.x_coords, self.y_coords) 
        self.dvs_dxs, self.dvs_dys = np.gradient(v_mean_squares, self.x_coords, self.y_coords) 
        self.dg1_dxs, self.dg1_dys = np.gradient(gamma1s, self.x_coords, self.y_coords) 
        self.dg2_dxs, self.dg2_dys = np.gradient(gamma2s, self.x_coords, self.y_coords) 
    ...
    def _corpus2deltav(self, i):
        ...
        t = torch.DoubleTensor(
            [
                [self.du_dxs[x_i][y_i], self.du_dys[x_i][y_i]], 
                [self.dv_dxs[x_i][y_i], self.dv_dys[x_i][y_i]]
            ]
        ) 
        return t 
    ...
    def q4(self, i): 
        ...
        partial_dp = [self.dp_dxs[x_i][y_i], self.dp_dys[x_i][y_i]] 
    ...
    def q6(self, i):
        ...
        rho = 1
        dU_square_dxs = [self.dus_dxs[x_i][y_i], self.dvs_dys[x_i][y_i]] 
    ...
    def q7(self, i):
        ...
        y_i = self.y_val2index[s[c2i['y']]] 
        U = [s[c2i['u_mean']], s[c2i['v_mean']]] 
        dU_dxs = [[self.du_dxs[x_i][y_i], self.du_dys[x_i][y_i]],
                 [self.dv_dxs[x_i][y_i], self.dv_dys[x_i][y_i]]] 
    ...
    def q10(self, i):
        ...
        y_i = self.y_val2index[s[c2i['y']]] 
        U = [s[c2i['u_mean']], s[c2i['v_mean']]] 
        dg_dxs = [[self.dg1_dys[x_i][y_i], self.dg1_dys[x_i][y_i]],
                [self.dg2_dxs[x_i][y_i], self.dg2_dys[x_i][y_i]]]
